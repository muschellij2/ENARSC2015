...
vignette: >
  ...
  %\VignetteIndexEntry{fslr and ANTsR Comparison}
output:
  knitr:::html_vignette:
    toc: yes
---

# FSL and ANTsR Pipeline
...
In the `fsl_pipeline` and `ANTsR_pipeline` vignettes, we analyzed MRI images of a person with a brain tumor from OsiriX (BRAINIX) [http://www.osirix-viewer.com/datasets/](http://www.osirix-viewer.com/datasets/).  Here, we will compare the two pipelines and use the registered-to-template images to create a table of areas the brain tumor engaged.
...
```{r atlas_breakdown, cache=FALSE, results='hide'}
